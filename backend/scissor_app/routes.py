import os, random, string, qr_codes, qrcode, base64, hashlib, io, validators, time, logging, urllib.parse, bcrypt
...
from .schemas import RegisterRequest, LoginRequest, UpdateRequest, UserResponse, VisitResponse, ShortenerRequest, QRResponse, VisitDetail, LinkHistoryResponse
from .dependencies import get_db, get_user_from_session, get_current_user, create_jwt_token, get_token
...
@app.get("/")
async def get_index():
    return {"message": "Welcome to Scissor URL Shortener!"}
...
def get_analytics(short_url: str, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        response_model = schemas.VisitResponse(
            ...
            visit_times=[schemas.VisitDetail(visit_time=visit.visit_time) for visit in visits],
            visit_count=url.visit_count,
            visits=[schemas.VisitDetail(visit_time=visit.visit_time) for visit in visits],
            url_user=url.user_id  
        )
...
@app.get("/link-history/{user_id}", response_model=list[schemas.LinkHistoryResponse])
def get_link_history(user_id: int, db: Session = Depends(get_db)):
    try:
...
async def register_user(user_request: RegisterRequest, db: Session = Depends(get_db)):
    try:
        ...
        if not all([user_request.user_fname, user_request.user_lname, user_request.email, user_request.pwd, user_request.cpwd]):
            raise HTTPException(status_code=400, detail="All fields are required")

        existing_user = db.query(Users).filter(Users.email == user_request.email).first()
        if existing_user:
            raise HTTPException(status_code=400, detail="Email already taken. Use another email address")

        hashedpwd = hash_password(user_request.pwd)

        new_user = Users(
            user_fname=user_request.user_fname,
            user_lname=user_request.user_lname,
        ...
        return {
            ...
            "user_lname": new_user.user_lname,
